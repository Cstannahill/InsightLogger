...
public sealed class AnalysisServiceAiEnrichmentTests
{
    ...
    public async Task AnalyzeAsync_Should_Apply_Ai_Enrichment_To_Primary_Candidate_When_Enrichment_Succeeds()
    {
        ...
        var service = CreateService(new[] { diagnostic }, new SuccessfulEnricher(), narrativeGenerator: null);
        ...
        result.Processing.Ai.Model.Should().Be("qwen3:8b");
        ...
        result.RootCauseCandidates[0].Explanation.Should().Be("AI-enriched explanation for the missing symbol.");
        result.RootCauseCandidates[0].LikelyCauses.Should().Contain("Typo in local variable name");
        result.RootCauseCandidates[0].SuggestedFixes.Should().Contain("Rename 'builderz' to the intended identifier.");
    ...
    {
        var diagnostic = CreateDiagnostic();
        var service = CreateService(new[] { diagnostic }, new FailedEnricher(), narrativeGenerator: null);
        ...
        result.RootCauseCandidates[0].LikelyCauses.Should().Contain("Typo in variable or member name");
        result.RootCauseCandidates[0].SuggestedFixes.Should().Contain("Check the symbol name for a typo.");
    ...
    public async Task AnalyzeAsync_Should_Apply_Ai_Narrative_For_MultiDiagnostic_Build_Log_When_Generation_Succeeds()
    {
        var diagnostics = new[]
        {
            CreateDiagnostic(id: "diag_ai_1", message: "The name 'builderz' does not exist in the current context", rawSnippet: "Program.cs(14,9): error CS0103: The name 'builderz' does not exist in the current context"),
            CreateDiagnostic(id: "diag_ai_2", message: "The name 'servicez' does not exist in the current context", rawSnippet: "Program.cs(15,9): error CS0103: The name 'servicez' does not exist in the current context")
        };

        var service = CreateService(diagnostics, new SuccessfulEnricher(), new SuccessfulNarrativeGenerator());
        ...
        result.Narrative.Summary.Should().Be("The build is failing because repeated missing-symbol errors point to one unresolved identifier pattern.");
        result.Narrative.GroupSummaries.Should().Contain("Missing symbol issues are clustered into one repeated fingerprint group.");
        result.RootCauseCandidates[0].Explanation.Should().Contain("cannot resolve");
        result.Warnings.Should().BeEmpty();
    }
    ...
    public async Task AnalyzeAsync_Should_Record_Separate_Ai_Tasks_When_Both_Toggles_Are_Enabled()
    {
        ...
        result.Processing.AiTasks.Should().Contain(task => task.Feature == "explanation-enrichment" && task.Status == "completed");
        result.Processing.AiTasks.Should().Contain(task => task.Feature == "root-cause-narrative" && task.Status == "completed");
        result.Narrative.Should().NotBeNull();
        result.Narrative!.Source.Should().Be("ai");
        result.RootCauseCandidates[0].Explanation.Should().Be("AI-enriched explanation for the missing symbol.");
    }
    ...
    public async Task AnalyzeAsync_Should_Keep_Deterministic_Narrative_When_Ai_Narrative_Fails()
    {
        ...
        var service = CreateService(diagnostics, new SuccessfulEnricher(), new FailedNarrativeGenerator());
    ...
    private static DiagnosticRecord CreateDiagnostic(string id = "diag_ai_1", string? message = null, string? rawSnippet = null)
        => new(
            id: id,
            ...
            rawSnippet: rawSnippet ?? "Program.cs(14,9): error CS0103: The name 'builderz' does not exist in the current context",
    ...
    {
        public Task<AiExplanationEnrichmentResult> EnrichAsync(ExplanationEnrichmentRequest request, CancellationToken cancellationToken = default)
            => Task.FromResult(AiExplanationEnrichmentResult.Successful(
    ...
    {
        public Task<AiExplanationEnrichmentResult> EnrichAsync(ExplanationEnrichmentRequest request, CancellationToken cancellationToken = default)
            => Task.FromResult(AiExplanationEnrichmentResult.Failure(
                status: "degraded",
                reason: "timeout",
                provider: "openrouter",
    ...
    {
        public Task<AiRootCauseNarrativeResult> GenerateAsync(RootCauseNarrativeRequest request, CancellationToken cancellationToken = default)
            => Task.FromResult(AiRootCauseNarrativeResult.Successful(
    ...
    {
        public Task<AiRootCauseNarrativeResult> GenerateAsync(RootCauseNarrativeRequest request, CancellationToken cancellationToken = default)
            => Task.FromResult(AiRootCauseNarrativeResult.Failure(
                status: "degraded",
                reason: "timeout",
                provider: "openrouter",
                model: "openai/gpt-5-mini"));
    ...
    private sealed class NoRulesRepository : IRuleRepository
    {
        ...
        public Task<Rule?> GetByIdAsync(string id, CancellationToken cancellationToken = default) => Task.FromResult<Rule?>(null);
        public Task<IReadOnlyList<Rule>> ListAsync(bool? isEnabled, ToolKind? toolKind, string? tag, int limit, int offset, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<Rule>>(Array.Empty<Rule>());
        public Task<int> CountAsync(bool? isEnabled, ToolKind? toolKind, string? tag, CancellationToken cancellationToken = default) => Task.FromResult(0);
        public Task<Rule> UpdateAsync(Rule rule, CancellationToken cancellationToken = default) => Task.FromResult(rule);
        public Task<IReadOnlyList<Rule>> GetEnabledRulesAsync(ToolKind? toolKind, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<Rule>>(Array.Empty<Rule>());
        public Task RecordMatchesAsync(IReadOnlyList<string> ruleIds, DateTimeOffset matchedAtUtc, CancellationToken cancellationToken = default) => Task.CompletedTask;
        public Task<IReadOnlyList<RelatedRuleSummaryDto>> GetRelatedRuleSummariesByFingerprintAsync(string fingerprint, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<RelatedRuleSummaryDto>>(Array.Empty<RelatedRuleSummaryDto>());
    }
